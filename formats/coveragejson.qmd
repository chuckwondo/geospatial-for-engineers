...
Section 2; for now, read it as "a field of values spread over space and/or
...
::: {.callout-important}
A coverage is a function that maps positions in space and/or time to values.
:::

![A coverage is a function: it maps a domain (positions in space and time) to a range (the measured values).](../diagrams/coverage-as-function.svg){#fig-coverage-function}
...
longitude, this latitude, at 3:00 p.m.") and the output is a number ("18.4
degrees Celsius").

A coverage is exactly that kind of function, made concrete: its **domain** is a
collection of positions in space and/or time, and its **range** is the set of
...
A useful mental picture: imagine an enormous spreadsheet laid over a map. The
rows and columns are positions on the ground; each filled cell holds a value. The
grid of cells is the domain; the values inside them are the range. If the coverage
measures two properties, picture two stacked sheets sharing the very same row and
column labels (one sheet of temperatures, one of humidities): the shared labels
are the single domain, and each sheet is a separate range. A coverage is the whole
spreadsheet-on-a-map, bundled with enough description that software can read any
cell unambiguously.
...
```text
    "parameters": {
┌─▶   "temperature": { ...what it is: air temperature, in Celsius... }
│   }
│   "ranges": {
└─▶   "temperature": { ...the temperature values: 18.4... }
    }

(the shared key "temperature" binds the description to its values)
```
...
side by side, exactly the picture of two stacked spreadsheet sheets sharing one
set of row and column labels from Section 2.1. To add a third property, you would
add a third matching pair of keys.
