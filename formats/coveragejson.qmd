...
**3. Composite values.** The first two forms describe a *grid*:
independent `x` and `y` axes combine by **cross-product** (every `x` paired with
every `y`), so three longitudes and two latitudes define 3 × 2 = six lattice
positions. That factoring only works when the positions truly fill a grid.
Irregular geometries do not: a trajectory visits specific `(x, y)` pairs in
sequence, *not* every `x` with every `y`, and there is no way to split those pairs
back into independent axes without inventing positions that were never sampled. So
a scattered set of points, a winding trajectory, or a polygon outline uses a single
axis, conventionally named `composite`, whose `values` are an **explicit list** of
the actual positions, each a *tuple* of coordinates rather than a single number.

![A grid laid out two ways. On the left, independent `x` and `y` axes cross-product to fill all nine cells. On the right, only three positions hold data, so the six cells a grid would invent stay hollow and a composite axis lists the three explicitly.](../diagrams/axes-independent-vs-composite.svg){#fig-composite-axis}

In the document, the two sides look like this. Two **independent axes** (the grid
on the left):

```json
"axes": {
  "x": { "values": [12.0, 12.5, 13.0] },
  "y": { "values": [45.0, 45.5, 46.0] }
}
```

or one **composite axis** (the three points on the right):
...
"axes": {
  "composite": {
    "dataType": "tuple",
    "coordinates": ["x", "y"],
    "values": [[12.0, 45.0], [12.5, 46.0], [13.0, 45.5]]
  }
}
```

Two fields say how to read those tuples. **`coordinates`** (here `["x", "y"]`)
names the members of each tuple *in order*, so in `[12.0, 45.0]` the first number
...
use a composite axis again in the trajectory example of Section 5.
