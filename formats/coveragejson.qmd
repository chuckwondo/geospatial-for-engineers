...
A domain needs at least `x` and `y` (the table in Section 4.1 lists them as always
required). Each individual axis writes its coordinates one of two ways: explicitly,
or -- when they are evenly spaced -- compactly.

**Explicit.** The form from Section 3: an object whose `values` array lists every
...
#### 4.2.2 Composite axes

Cross-product only works when the positions truly fill a grid. A **non-gridded**
coverage (Section 2.2) does not: a scatter of stations, a ship's winding path, or a
